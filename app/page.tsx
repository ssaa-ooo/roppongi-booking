...
export default function Home() {
  ...
  // 時間が選択されたらフォームへスクロール
  useEffect(() => {
    if (formData.startTime && formRef.current) {
      setTimeout(() => {
  ...
  const handleSlotClick = (time: string, seatIndex: number, isAvailable: boolean) => {
    if (!isAvailable) return;
    setFormData({ ...formData, startTime: time, duration: 60 });
    setSelectedSeat(seatIndex); // 特定の席だけを選択状態にする
    setStatus(""); 
  };
            ...
            <div className="space-y-8">
              ...
              <div className="overflow-x-auto pb-2">
                        ...
                        return (
                          <div key={time} className="grid grid-cols-7 border-b border-gray-200 last:border-b-0 h-12">
                            ...
                            {[...Array(MAX_CAPACITY)].map((_, i) => {
                              ...
                              // ★修正点：行だけでなく、席番号(i)も一致している場合のみ「選択中」にする
                              const isSelected = isSelectedRow && (selectedSeat === i);
                              
                              return (
                                <div 
                                  key={i} 
                                  onClick={() => handleSlotClick(time, i, isAvailable)}
                                  className={`
                                    ...
                                    ${isBooked 
                                      ...
                                      : isSelected 
                                        ? "bg-[#007AC3] text-white scale-95 rounded-sm shadow-md z-10" 
                ...
                <div className="flex justify-center gap-6 mt-3 text-xs text-gray-600">
                  ...
                  <div className="flex items-center"><span className="w-4 h-4 bg-gray-300 mr-2 rounded-sm"></span> 予約済</div>
                  <div className="flex items-center"><span className="w-4 h-4 bg-[#007AC3] mr-2 rounded-sm"></span> 選択中</div>
                </div>
              </div>

              {/* 予約フォーム (ここに自動スクロールします) */}
              <div ref={formRef}>
                {formData.startTime && (
